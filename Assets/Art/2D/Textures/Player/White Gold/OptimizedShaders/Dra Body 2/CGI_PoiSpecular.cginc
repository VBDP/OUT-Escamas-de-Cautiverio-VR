    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(1) == 1) // Realistic
        {
            ...
            if(float(1))
            {
                ...
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.457) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
            #else
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.457) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
            #endif
        }
        if(float(1) == 4)
        {
            ...
            if (float(1) == 3) // anisotropic
            {
                ...
                jitter += float(0);
                float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.457), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(1,1,1,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(1) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.457) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
